...
class Tm_Builder_Module_Taxonomy extends Tm_Builder_Module {
	...
	public function get_items(){
		...
		foreach ( $terms as $term_key => $term ){
			...
			$title = tm_builder_core()->utility()->attributes->get_title(
				apply_filters( 'tm_pb_module_taxonomy_title_settings',
					array(
						...
						'length'		=> $title_length,
						'html'			=> '<h6 %1$s><a href="' . $permalink . '" %3$s>%4$s</a></h6>',
					),
					$this
				),
				'term',
			...
			$count_sufix = _n_noop( '%s post', '%s posts', '__tm' );

			if ( 'product_cat' === $terms_type ) {
				$count_sufix = _n_noop( '%s product', '%s products', '__tm' );
			}
			...
			$button = tm_builder_core()->utility()->attributes->get_button(
				apply_filters( 'tm_pb_module_taxonomy_button_settings',
					array(
						...
						'text'		=> $button_text,
						'html'		=> '<span class="button--holder"><a href="' . $permalink . '" %2$s %3$s><span class="btn__text">%4$s</span>%5$s</a></span>',
